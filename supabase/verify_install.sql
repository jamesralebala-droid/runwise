...
declare
  ...
  expected_tables text[] := array[
    ...
    'live_locations','legal_documents','legal_acceptances','legal_compliance_flags'
  ];
  expected_functions text[] := array[
    ...
    'prevent_legal_document_edit','archive_previous_legal_version','has_accepted_current'
  ];
  t text;
  f text;
begin
...
-- Should show 10 rows, all status = 'published'.
select document_type, version, status from legal_documents order by document_type;
