...
export async function getOpenTrips(): Promise<Trip[]> {
  const { data, error } = await supabase
    .from('trips')
    .select('*')
    .in('status', ['leaving_soon', 'today', 'tomorrow', 'upcoming'])
    .order('depart_date')
    .limit(50);
...
export async function recordAcceptance(
  ...
  if (error) {
    // Non-fatal: legal tables may not exist in all environments
    console.warn('recordAcceptance skipped:', error.message);
  }
}
