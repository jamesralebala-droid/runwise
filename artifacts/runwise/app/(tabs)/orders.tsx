  ...
  return (
    <Screen>
      <Header
        eyebrow="ORDER ROOMS"
        title="My orders"
        subtitle="Payment, chat, live milestones and delivery confirmation stay together here."
      />
        ...
        return (
          <Card key={order.id}>
            <View style={styles.row}>
              ...
              <Pill text={titleCase(escrow?.status || 'pending')} tone={settled ? 'success' : escrow?.status === 'disputed' ? 'danger' : 'warning'} />
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.muted}>Order total</Text>
              <Text style={styles.total}>{formatMoney(escrow?.total)}</Text>
            </View>
            <AppButton title="Open Order Room" onPress={() => router.push({ pathname: '/order/[id]', params: { id: order.id } })} />
          </Card>
        );
