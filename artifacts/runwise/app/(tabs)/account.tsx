...
export default function AccountScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [walletData, verificationData, vehicleData] = await Promise.all([
        getWallet(profile.id), getVerification(profile.id), getVehicles(profile.id),
      ]);
  ...
  const withdraw = async () => {
    ...
    setWithdrawAmount('');
    Alert.alert('Request received', 'Your withdrawal request has been logged. Processing via the admin panel.');
    load();
  };
  ...
  return (
    <Screen>
      <Header
        eyebrow={runner ? 'RUNNER ACCOUNT' : 'CUSTOMER ACCOUNT'}
        title={profile?.full_name || 'Account'}
        subtitle={`★ ${rating} • RunScore ${profile?.run_score} • ${titleCase(profile?.run_score_level || 'bronze')}`}
      />
      <Card>
      ...
      {runner && (
        <Card>
          <View style={styles.row}>
            <Text style={styles.sectionTitle}>Runner setup</Text>
            <Pill text={verification?.status || 'not submitted'} tone={verification?.status === 'approved' ? 'success' : verification?.status === 'rejected' ? 'danger' : 'warning'} />
          </View>
          <Text style={styles.muted}>{vehicles.filter((v) => v.approved).length} approved vehicle(s)</Text>
          <View style={styles.modeRow}>
            <AppButton title="Verification" variant="secondary" onPress={() => router.push('/verification')} style={{ flex: 1 }} />
            <AppButton title="Vehicles" variant="secondary" onPress={() => router.push('/vehicles')} style={{ flex: 1 }} />
          </View>
        </Card>
      )}
      {runner && (
        <Card>
          ...
          <View style={styles.methodRow}>
            {([['orange_money', 'Orange Money'], ['myzaka', 'MyZaka'], ['bank_transfer', 'Bank transfer']] as const).map(([value, label]) => (
              <Pressable key={value} onPress={() => setWithdrawMethod(value)} style={[styles.method, withdrawMethod === value && styles.methodActive]}>
                <Text style={[styles.methodText, withdrawMethod === value && styles.methodTextActive]}>{label}</Text>
              </Pressable>
            ))}
          </View>
          <AppButton title="Request withdrawal" onPress={withdraw} variant="secondary" loading={busy} />
          <Text style={styles.muted}>Withdrawals are reviewed and processed via the admin panel.</Text>
        </Card>
      )}
      <Card>
        ...
        {transactions.slice(0, 8).map((tx) => (
          <View key={tx.id} style={styles.transaction}>
            <Text style={styles.transactionName}>{titleCase(tx.type)}</Text>
            <Text style={[styles.transactionAmount, { color: Number(tx.amount) < 0 ? colors.danger : colors.success }]}>
              {Number(tx.amount) < 0 ? '−' : '+'}{formatMoney(Math.abs(Number(tx.amount)))}
            </Text>
          </View>
        ))}
      ...
      <AppButton
        title="Sign out"
        variant="danger"
        onPress={() => Alert.alert('Sign out?', 'You can sign back in at any time.', [
...
function ModeButton({ title, active, disabled, onPress }: { title: string; active: boolean; disabled: boolean; onPress: () => void }) {
  return (
    <Pressable disabled={disabled} onPress={onPress} style={[styles.mode, active && styles.modeActive]}>
