...
export default function ActivityScreen() {
  ...
  return (
    <Screen>
      <Header
        eyebrow={runner ? 'RUNNER' : 'CUSTOMER'}
        title={runner ? 'My trips' : 'My requests'}
        subtitle={runner ? 'Manage the journeys you have announced.' : 'Track everything you have asked a runner to carry or buy.'}
        action={<AppButton title={runner ? '+ Trip' : '+ Request'} variant="secondary" disabled={!!profile?.restricted} onPress={() => router.push(runner ? '/trip/new' : '/request/new')} />}
      />
      {items.length
        ? items.map((item) => runner
          ? <TripItem key={item.id} trip={item as Trip} />
          : <RequestCard key={item.id} request={item as RequestItem} />)
        : <EmptyState
            icon={runner ? '🚙' : '📦'}
            title={runner ? 'No trips announced' : 'No requests posted'}
            message={runner ? 'Announce your route and RunWise will find requests along the way.' : 'Post your first request and let a runner bring it.'}
          />}
    </Screen>
  );
}
...
function RequestCard({ request }: { request: RequestItem }) {
  return (
    <Card>
      <View style={styles.row}><Text style={styles.icon}>{REQUEST_ICONS[request.type]}</Text><Pill text={request.status} tone={request.status === 'open' ? 'warning' : 'success'} /></View>
      <Text style={styles.route}>{request.from_city} → {request.to_city}</Text>
      <Text style={styles.muted}>{titleCase(request.type)} • {formatMoney(request.estimated_value)}</Text>
...
function TripItem({ trip }: { trip: Trip }) {
  return (
    <Card>
      ...
      <Text style={styles.muted}>{trip.spaces_remaining}/{trip.capacity_spaces} spaces • {trip.capacity_kg} kg • Potential {formatMoney(trip.potential_earnings)}</Text>
    </Card>
  );
}
