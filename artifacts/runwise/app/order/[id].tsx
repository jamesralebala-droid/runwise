...
export default function OrderRoomScreen() {
  ...
  const fundEscrow = () => {
    ...
    run('fund', async () => {
      ...
      if (rpcError) throw rpcError;
    }, 'Escrow funded using the demo payment method.');
  };
  const savePin = () => {
    if (!/^\d{4,6}$/.test(pin)) return Alert.alert('Invalid PIN', 'Use a private 4–6 digit PIN.');
    run('pin', async () => {
      const { error: rpcError } = await supabase.rpc('set_delivery_pin', { p_order_room_id: id, p_pin: pin });
      if (rpcError) throw rpcError;
      setPin('');
    }, 'Delivery PIN saved. Keep it private until you receive the delivery.');
  };
  const confirmDelivery = () => {
    if (!/^\d{4,6}$/.test(deliveryPin)) return Alert.alert('PIN required', 'Enter your 4–6 digit delivery PIN.');
    Alert.alert('Confirm delivery?', 'This releases escrow after the server validates your PIN.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Confirm and release', onPress: () => run('delivery', async () => {
        const { error: rpcError } = await supabase.rpc('confirm_delivery', { p_order_room_id: id, p_pin: deliveryPin, p_actual_spent: actualSpent ? Number(actualSpent) : null });
        if (rpcError) throw rpcError;
  ...
  const raiseDispute = () => {
    ...
    Alert.alert('Raise dispute?', 'Escrow will be frozen while an administrator reviews the order.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Raise dispute', style: 'destructive', onPress: () => run('dispute', async () => {
        const { error: rpcError } = await supabase.rpc('raise_dispute', { p_order_room_id: id, p_reason: disputeReason, p_evidence: { details: disputeDetails.trim() } });
        if (rpcError) throw rpcError;
      }, 'Dispute raised. Escrow is now frozen.') },
    ]);
  };
  const shareLocation = () => run('location', async () => {
    ...
    const { error: locationError } = await supabase.from('live_locations').upsert(
      { order_room_id: id, user_id: profile.id, latitude: current.coords.latitude, longitude: current.coords.longitude, updated_at: new Date().toISOString() },
      { onConflict: 'order_room_id,user_id' },
    );
  ...
  const submitRating = () => run('rating', async () => {
    ...
    const { error: ratingError } = await supabase.from('ratings').insert({
      order_room_id: id, rater_id: profile.id, ratee_id: rateeId, stars, areas: {}, comment: ratingComment.trim() || null,
    });
  ...
  return (
    <Screen>
      <View style={styles.headingRow}>
        <View><Text style={styles.eyebrow}>ORDER {id.slice(0, 8).toUpperCase()}</Text><Text style={styles.heading}>Order Room</Text></View>
        <Pill text={titleCase(escrow?.status || 'pending')} tone={isDisputed ? 'danger' : settled ? 'success' : 'warning'} />
      </View>
      <Card>
        <Text style={styles.title}>Escrow</Text>
        <MoneyRow label="Item value" value={escrow?.item_value} />
        <MoneyRow label="Runner fee" value={escrow?.runner_fee} />
        <MoneyRow label="Platform fee" value={escrow?.platform_fee} />
        <MoneyRow label="Protection fee" value={escrow?.protection_fee} />
        <View style={styles.total}>
          <Text style={styles.totalLabel}>Total</Text>
          <Text style={styles.totalValue}>{formatMoney(escrow?.total)}</Text>
        </View>
      ...
      <Card>
        ...
        {milestones.map((item, index) => (
          <View key={item.id} style={styles.timeline}>
            <View style={styles.dot}><Text style={styles.dotText}>{index + 1}</Text></View>
            <View style={{ flex: 1 }}>
              <Text style={styles.timelineTitle}>{MILESTONES[item.milestone] || titleCase(item.milestone)}</Text>
              <Text style={styles.small}>{new Date(item.created_at).toLocaleString()}{item.note ? ` • ${item.note}` : ''}</Text>
            </View>
          </View>
        ))}
        {!milestones.length && <Text style={styles.muted}>No milestones posted yet.</Text>}
        {!isCustomer && !isDisputed && <>
          <Text style={styles.label}>Post milestone</Text>
          <View style={styles.choiceWrap}>
            {Object.entries(MILESTONES).map(([value, label]) => (
              <Pressable key={value} onPress={() => setMilestone(value)} style={[styles.choice, milestone === value && styles.choiceActive]}>
                <Text style={[styles.choiceText, milestone === value && styles.choiceTextActive]}>{label}</Text>
              </Pressable>
            ))}
          </View>
          <AppButton title="Post milestone" variant="secondary" onPress={postMilestone} loading={busy === 'milestone'} />
        </>}
      </Card>
      <Card>
        <Text style={styles.title}>Order chat</Text>
        {messages.map((item) => (
          <View key={item.id} style={[styles.bubble, item.sender_id === profile.id ? styles.mine : styles.theirs]}>
            <Text style={styles.bubbleName}>{item.sender_id === profile.id ? 'You' : 'Other party'}</Text>
      ...
      {active && (
        <Card>
          <Text style={styles.title}>Safe handover</Text>
          <Text style={styles.muted}>Both parties share their current location. A phone number is revealed only when you are close enough.</Text>
          <View style={styles.actionRow}>
            <AppButton title="Share location" variant="secondary" onPress={shareLocation} loading={busy === 'location'} style={{ flex: 1 }} />
            <AppButton title="Check distance" variant="secondary" onPress={checkNearby} loading={busy === 'nearby'} style={{ flex: 1 }} />
          </View>
          {!!nearby && <Text style={styles.nearby}>{nearby}</Text>}
        </Card>
      )}
      {isCustomer && active && !isDisputed && (
        <Card>
          <Text style={styles.title}>Delivery PIN</Text>
          <Text style={styles.muted}>Create a private PIN and enter it only after you receive the delivery.</Text>
      ...
      {dispute ? (
        <Card style={styles.dispute}>
          <View style={styles.headingRow}>
            <Text style={styles.title}>Dispute</Text>
            <Pill text={dispute.status} tone={dispute.status === 'resolved' ? 'success' : 'danger'} />
          </View>
          <Text style={styles.muted}><Text style={styles.bold}>Reason: </Text>{dispute.reason}</Text>
          <Text style={styles.muted}>{dispute.status === 'resolved' ? `Resolution: ${dispute.resolution || 'Completed'}` : 'Escrow is frozen while an administrator reviews this order.'}</Text>
        </Card>
      ) : active && (
        <Card>
          ...
          <View style={styles.choiceWrap}>
            {DISPUTE_REASONS.map((reason) => (
              <Pressable key={reason} onPress={() => setDisputeReason(reason)} style={[styles.choice, disputeReason === reason && styles.choiceDanger]}>
                <Text style={[styles.choiceText, disputeReason === reason && styles.choiceDangerText]}>{reason}</Text>
              </Pressable>
            ))}
          </View>
          <Field label="What happened?" value={disputeDetails} onChangeText={setDisputeDetails} multiline />
          <AppButton title="Raise a dispute" variant="danger" onPress={raiseDispute} loading={busy === 'dispute'} />
        </Card>
      )}
      {settled && (
        <Card>
          ...
          {myRating ? (
            <Text style={styles.muted}>You rated this order {myRating.stars}/5. Thank you.</Text>
          ) : <>
            ...
            <Field label="Comment (optional)" value={ratingComment} onChangeText={setRatingComment} multiline />
            <AppButton title="Submit rating" onPress={submitRating} loading={busy === 'rating'} />
          </>}
        </Card>
      )}
    </Screen>
...
function MoneyRow({ label, value }: { label: string; value: number | undefined }) {
  return <View style={styles.moneyRow}><Text style={styles.muted}>{label}</Text><Text style={styles.money}>{formatMoney(value)}</Text></View>;
}
