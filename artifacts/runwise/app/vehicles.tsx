...
export default function VehiclesScreen() {
  ...
  const load = useCallback(async () => {
    if (!profile) return;
    setLoading(true);
    try { setVehicles(await getVehicles(profile.id)); }
    catch (err) { Alert.alert('Could not load vehicles', friendlyError(err)); }
  ...
  const submit = async () => {
    if (!profile || !makeModel.trim() || !plate.trim() || !photos.length)
      return Alert.alert('Missing information', 'Enter the vehicle details and select at least one photo.');
    setBusy(true);
    try {
      const paths: string[] = [];
      for (const photo of photos) paths.push(await uploadAsset(profile.id, 'vehicles', photo));
      const { error } = await supabase.from('vehicles').insert({
        id: newId(), user_id: profile.id, make_model: makeModel.trim(),
        plate_number: plate.trim().toUpperCase(), photo_urls: paths, approved: false, review_status: 'pending',
      });
        ...
        return (
          <Card key={vehicle.id}>
            <View style={styles.row}>
              <Text style={styles.vehicle}>{vehicle.make_model}</Text>
              <Pill text={status === 'pending' ? 'pending approval' : status} tone={status === 'approved' ? 'success' : status === 'rejected' ? 'danger' : 'warning'} />
            </View>
            ...
            {status === 'rejected' && (
              <View style={styles.feedback}>
                ...
                <Text style={styles.feedbackText}>{vehicle.rejection_reason || 'Please correct the vehicle details or photos and submit a new vehicle.'}</Text>
              </View>
            )}
          </Card>
        );
...
const styles = StyleSheet.create({
  title: { color: colors.green, fontWeight: '900', fontSize: 20 },
  vehicle: { color: colors.green, fontWeight: '900', fontSize: 18, flex: 1 },
