...
export default function NewRequestScreen() {
  ...
  const submit = async () => {
    ...
    const id = newId();
    try {
      const { error } = await supabase.from('requests').insert({ id, customer_id: profile.id, type, from_city: fromCity.trim(), to_city: toCity.trim(), estimated_value: Number(value || 0), from_landmark: fromLandmark.trim() || null, to_landmark: toLandmark.trim() || null, details: details.trim() || null, written_directions: directions.trim() || null });
      ...
      Alert.alert('Request posted', 'Runners on matching routes can now send you an offer.', [{ text: 'Done', onPress: () => router.replace('/(tabs)/activity') }]);
  ...
  return (
    <Screen>
      ...
      <View style={styles.types}>{REQUEST_TYPES.map((item) => <Pressable key={item} onPress={() => setType(item)} style={[styles.type, type === item && styles.typeActive]}><Text style={styles.typeIcon}>{REQUEST_ICONS[item]}</Text><Text style={[styles.typeText, type === item && styles.typeTextActive]}>{REQUEST_LABELS[item]}</Text></Pressable>)}</View>
      <Card>
...
function CheckRow({ checked, onPress, text }: { checked: boolean; onPress: () => void; text: string }) {
  return <Pressable onPress={onPress} style={styles.checkRow}><View style={[styles.checkbox, checked && styles.checkboxActive]}><Text style={styles.check}>{checked ? '✓' : ''}</Text></View><Text style={styles.checkText}>{text}</Text></Pressable>;
}
