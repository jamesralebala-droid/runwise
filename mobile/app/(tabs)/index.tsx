...
const CUSTOMER_TIPS = [
  {
    icon: '📅',
    title: 'Post early for more options',
    message: 'Post your request a few days before you need it. That gives upcoming runners more time to see and match your route.',
  },
  {
    icon: '🌱',
    title: 'RunWise is growing',
    message: 'A quiet trip board does not mean you did anything wrong. Keep your request posted and check again as new runners join.',
  },
  {
    icon: '🔒',
    title: 'Your contact details stay private',
    message: 'Your phone number is not shown publicly. During an active order, it can only be revealed when both parties are close enough for handover.',
  },
  {
    icon: '📍',
    title: 'Location sharing is controlled by you',
    message: 'RunWise does not track you continuously. Location is shared only when you choose Share location during an active handover.',
  },
  {
    icon: '📝',
    title: 'Clear details improve matching',
    message: 'Add useful landmarks, directions, item size and timing so a runner can quickly decide whether the request fits their journey.',
  },
  {
    icon: '🛡️',
    title: 'Keep the delivery PIN private',
    message: 'Give the PIN only after you have received and checked your delivery. It protects the final escrow release.',
  },
];
...
const RUNNER_TIPS = [
  {
    icon: '🗓️',
    title: 'Announce journeys in advance',
    message: 'Post an upcoming route a few days early. Customers then have more time to find your trip and prepare suitable requests.',
  },
  {
    icon: '🌱',
    title: 'Early runners help RunWise grow',
    message: 'RunWise is new, so some routes may be quiet at first. Keep genuine upcoming trips posted; matching opportunities can grow as customers join.',
  },
  {
    icon: '🚙',
    title: 'Keep vehicle details current',
    message: 'Use only the approved vehicle shown on your trip. Accurate capacity and space information helps customers choose safely.',
  },
  {
    icon: '🔒',
    title: 'Your number stays private',
    message: 'Your phone number is not displayed publicly. During an active order, it can only be revealed when both parties are close enough for handover.',
  },
  {
    icon: '📍',
    title: 'You control location sharing',
    message: 'RunWise does not track you continuously. Share your current location only from the active Order Room when coordinating a safe handover.',
  },
  {
    icon: '✅',
    title: 'Post honest milestones',
    message: 'Simple, timely updates reduce worry for customers and help build your rating and RunScore over time.',
  },
];
    ...
    <Screen>
      ...
      <Card style={styles.guide}>
        <View style={styles.guideTop}>
          <View style={styles.guideTitleWrap}><Text style={styles.guideIcon}>{tip.icon}</Text><View style={{ flex: 1 }}><Text style={styles.guideEyebrow}>RUNWISE GUIDE</Text><Text style={styles.guideTitle}>{tip.title}</Text></View></View>
          <Text style={styles.tipCount}>{(tipIndex % tips.length) + 1}/{tips.length}</Text>
        </View>
        <Text style={styles.guideMessage}>{tip.message}</Text>
        <Pressable accessibilityRole="button" accessibilityLabel="Show next RunWise tip" onPress={() => setTipIndex((current) => current + 1)} style={({ pressed }) => [styles.nextTip, pressed && styles.nextTipPressed]}>
