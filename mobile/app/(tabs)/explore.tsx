      ...
      {tab === 'trips' ? (
        ...
        activeOffers.length ? activeOffers.map((offer) => (
          <Card key={offer.id}>
            ...
            {['proposed', 'accepted_by_runner'].includes(offer.status) && <View style={styles.actions}><AppButton title="Decline" variant="secondary" onPress={() => respond(offer.id, false)} loading={busy === offer.id} style={{ flex: 1 }} /><AppButton title="Accept offer" onPress={() => respond(offer.id, true)} loading={busy === offer.id} style={{ flex: 1 }} /></View>}
          </Card>
...
function RunnerMatches() {
  ...
  useFocusEffect(useCallback(() => { load(); }, [load]));
  const compatible = useMemo(() => requests.map((request) => ({ request, trip: trips.find((trip) => trip.from_city.toLowerCase() === request.from_city.toLowerCase() && trip.to_city.toLowerCase() === request.to_city.toLowerCase()) })).filter((match) => match.trip), [requests, trips]);
  ...
  return (
    <Screen>
      <Header eyebrow="SMART MATCHES" title="Requests on your route" subtitle="Only requests matching your announced start and destination are shown." action={<AppButton title="+ Trip" onPress={() => router.push('/trip/new')} variant="secondary" />} />
      ...
      )) : <EmptyState icon="🧭" title="No matching requests" message={trips.length ? 'There are no open requests along your announced routes yet.' : 'Announce a trip first so RunWise can find requests along your route.'} action={!trips.length ? <AppButton title="Announce a trip" onPress={() => router.push('/trip/new')} /> : undefined} />}
    </Screen>
  );
}
...
function TripCard({ trip }: { trip: Trip }) {
  ...
  return (
    <Card>
      ...
      <View style={styles.pills}>{trip.services.map((service) => <Pill key={service} text={titleCase(service)} />)}</View>
