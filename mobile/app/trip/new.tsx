...
export default function NewTripScreen() {
  ...
  if (!vehicles.length) return <Screen><Card style={styles.gate}><Text style={styles.gateIcon}>🚙</Text><Text style={styles.gateTitle}>Approved vehicle required</Text><Text style={styles.muted}>Add a vehicle and wait for approval before announcing a trip.</Text><AppButton title="Open my vehicles" onPress={() => router.replace('/vehicles')} /></Card></Screen>;
  ...
  const submit = async () => {
    if (!profile) return;
    if (!vehicleId || !fromCountry.trim() || !fromCity.trim() || !toCountry.trim() || !toCity.trim() || !date || !time) return Alert.alert('Missing information', 'Complete the vehicle, route, date and time.');
  ...
  return (
    <Screen>
      ...
      <View style={styles.vehicles}>{vehicles.map((vehicle) => <Pressable key={vehicle.id} onPress={() => setVehicleId(vehicle.id)} style={[styles.vehicle, vehicleId === vehicle.id && styles.selected]}><Text style={styles.vehicleName}>{vehicle.make_model}</Text><Text style={styles.muted}>{vehicle.plate_number || 'No plate'}</Text>{vehicleId === vehicle.id && <Pill text="Selected" tone="success" />}</Pressable>)}</View>
      <Card>
      ...
      <Pressable onPress={() => setDeclared(!declared)} style={styles.declaration}><View style={[styles.checkbox, declared && styles.checkboxActive]}><Text style={styles.check}>{declared ? '✓' : ''}</Text></View><Text style={styles.declarationText}>I am legally permitted to drive and use this vehicle. I will follow road, insurance, border and customs laws, and reject dangerous or unlawful goods.</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  vehicles: { gap: 8 },
  vehicle: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, padding: 13, gap: 4 },
  vehicleName: { color: colors.green, fontWeight: '900' },
  ...
  service: { borderWidth: 1, borderColor: colors.border, borderRadius: radius.pill, backgroundColor: colors.white, paddingVertical: 9, paddingHorizontal: 12 },
  serviceText: { color: colors.muted, fontWeight: '700' },
  ...
  declarationText: { flex: 1, color: colors.charcoal, lineHeight: 20 },
